...
export async function listDrinks(input: ListDrinksInput): Promise<{ total_record: number; data: DrinkObject[] }> {
  const { total_record, items } = await findDrinks(input);
  return { total_record, data: items.map(toDrinkObject) };
}
