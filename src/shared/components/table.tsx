...
import { SortDirectionIcon } from "./sort-direction-icon";
...
type BaseTableStyleProps<T extends RowData = RowData> = {
  // Container styles
...
type BaseTableProps<T extends RowData> = BaseTableStyleProps<T> & {
  // Core data props
  data: T[];
  columns: ColumnDef<T>[];
  isLoading?: boolean;
  ...
  groupDetailPanel?: boolean;
  renderDetailPanel?: (row: Row<T>) => React.ReactNode;
...
export function Table<T extends RowData>({
  ...
  columns,
  isLoading = false,
  // isFetching = false, TODO: wait for progress component to be added
  ...
  const handleRowClick = useCallback(
    (row: Row<T>) => (e: React.MouseEvent<HTMLTableRowElement, MouseEvent>) => {
      ...
      if (renderDetailPanel && row.getCanExpand()) {
        row.toggleExpanded();
      }
    },
    [onRowClick, renderDetailPanel]
  );
  ...
  const showFooter = (selectedCount && selectedCount > 0) || showPagination;
                  ...
                  return (
                    <React.Fragment key={row.id}>
                        ...
                        {cells.map((cell) => {
                          ...
                          return (
                            <td
                              ...
                              style={{
                                minWidth: `${cell.column.getSize() / 16}rem`,
                              }}
                            >
                              {label}
                            </td>
                      ...
                      {row.getIsExpanded() &&
                        ...
                        (groupDetailPanel ? (
                          <tr
                            className={cn(baseRowClassNames({ variant: rowVariant }), rowClassName)}
                          >
                            <td
