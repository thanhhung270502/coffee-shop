...
import { ArrowLeftIcon, ArrowRightIcon } from "../external-icons";
...
function Sidebar({
  ...
  if (collapsible === "none") {
    return (
      <div
        data-slot="sidebar"
        className={cn(
          "bg-blue-primary text-primary flex h-full w-(--sidebar-width) flex-col",
          className
        )}
        {...props}
      >
    ...
    return (
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        <SheetContent
          ...
          data-tablet="true"
          showOverlay={false}
          className="bg-blue-primary text-primary px-md py-2xl w-(--sidebar-width) shadow-xl [&>button]:hidden"
          style={
            {
  ...
  return (
    <div
      className={cn("group peer bg-blue-primary block", {
        ...
        className={cn(
          ...
          side === "left"
            ...
            : side === "right"
              ...
              : "bottom-0 group-data-[collapsible=offcanvas]:-bottom-(--sidebar-height-bottom)",
          side === "bottom"
            ? "bg-blue-primary h-(--sidebar-height-bottom) w-full"
...
function SidebarTrigger({ className, onClick, iconClassName, ...props }: SidebarTriggerProps) {
  ...
  return (
    <button
      ...
      className={cn("bg-blue-main cursor-pointer rounded-full p-1", className)}
      onClick={(event) => {
        onClick?.(event);
        toggleSidebar();
      }}
      {...props}
    >
      <Icon className={cn("text-white", iconClassName)} size="1.375rem" />
...
function SidebarRail({ className, ...props }: ComponentProps<"button">) {
  ...
  return (
    <button
      ...
      className={cn(
        "hover:after:bg-blue-secondary-hover absolute inset-y-0 z-20 hidden w-4 -translate-x-1/2 transition-all ease-linear group-data-[side=left]:-right-4 group-data-[side=right]:left-0 after:absolute after:inset-y-0 after:left-1/2 after:w-[2px] sm:flex",
        "in-data-[side=left]:cursor-w-resize in-data-[side=right]:cursor-e-resize",
        "[[data-side=left][data-state=collapsed]_&]:cursor-e-resize [[data-side=right][data-state=collapsed]_&]:cursor-w-resize",
        "hover:group-data-[collapsible=offcanvas]:bg-blue-secondary-hover group-data-[collapsible=offcanvas]:translate-x-0 group-data-[collapsible=offcanvas]:after:left-full",
...
function SidebarGroupAction({
  ...
  return (
    <Comp
      ...
      className={cn(
        "text-primary hover:bg-blue-secondary-hover hover:text-primary absolute top-3.5 right-3 flex aspect-square w-5 items-center justify-center rounded-md p-0 outline-hidden transition-transform focus-visible:ring-2 [&>svg]:size-4 [&>svg]:shrink-0",
...
const sidebarMenuButtonVariants = cva(
  ...
  ],
  {
    variants: {
      variant: {
        default: [
          "text-primary hover:text-primary active:text-blue-tertiary data-[active=true]:text-blue-tertiary",
          "bg-transparent hover:bg-blue-secondary-hover active:bg-blue-secondary data-[active=true]:bg-blue-secondary",
...
function SidebarMenuIcon({
  ...
  return (
    <div
      ...
      className={cn(
        "p-md flex size-8 items-center justify-center rounded-4xl border group-data-[collapsible=icon]:size-8",
        "border-secondary text-tertiary bg-white",
        "group-data-[active=true]:bg-blue-main group-data-[active=true]:text-white",
        className
      )}
      {...props}
    >
      <ItemIcon variant="Bold" size="1rem" />
    </div>
  );
}
...
function SidebarMenuSub({ className, ...props }: ComponentProps<"ul">) {
  return (
    <ul
      ...
      className={cn(
        "border-secondary ml-6.5 flex min-w-0 translate-x-px flex-col border-l",
        "group-data-[collapsible=icon]:hidden",
        className
      )}
      {...props}
...
function SidebarMenuSubButton({
  ...
  return (
    <span
      ...
      data-active={isActive}
      className={cn(
        "body-md text-primary hover:text-primary active:text-blue-tertiary pr-xl py-md flex h-9 min-w-0 -translate-x-px items-center overflow-hidden pl-6 font-medium outline-hidden focus-visible:ring-2 disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50",
        "border-brand-subtle data-[active=true]:border-brand border-l",
        "data-[active=true]:text-blue-tertiary",
        "group-data-[collapsible=icon]:hidden",
        className
      )}
      {...props}
...
function SecondarySidebar({
    ...
    return (
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        <SheetContent
          ...
          data-tablet="true"
          showOverlay={false}
          className="bg-blue-primary text-primary w-(--sidebar-width) shadow-xl [&>button]:hidden"
          style={
            {
              "--sidebar-width": SIDEBAR_WIDTH_MOBILE,
            } as CSSProperties
  ...
  if (isNotDesktop && !openMobile) {
    return (
      <div className="fixed bottom-20 left-4 z-10">
        <SidebarTrigger className="bg-blue-main p-sm" iconClassName="text-white" />
      </div>
    );
  }
  ...
  return (
    <div
      className={cn("group peer bg-blue-primary relative block", {
        "h-(--sidebar-height-bottom)": isMobile,
      })}
      data-state={state}
      data-collapsible={state === "collapsed" ? collapsible : ""}
    ...
    >
      ...
      <div
        ...
        className={cn(
          "transition-[left,right,bottom,width, height] fixed z-10 h-dvh duration-200 ease-linear md:flex",
          side === "left"
            ? "border-brand-subtle left-(--sidebar-width-icon) border-l group-data-[collapsible=offcanvas]:-left-(--sidebar-width)"
            : side === "right"
              ? "right-0 group-data-[collapsible=offcanvas]:-right-(--sidebar-width)"
              : "bottom-(--sidebar-height-bottom) group-data-[collapsible=offcanvas]:-bottom-(--sidebar-height-bottom)",
          side === "bottom"
            ? "bg-blue-primary h-(--sidebar-height-bottom) w-full"
            : "inset-y-0 w-(--sidebar-width) group-data-[collapsible=icon]:w-(--sidebar-width-icon) group-data-[side=right]:border-l",
          className
